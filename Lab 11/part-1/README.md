...
1. With your partner, edit the `states_functions.cc` source file using VS Code. Replace all the TODO comments with working code.
...
This lab is following this practice. The `State` class is declared in the header file `states_functions.h`, and
the function definitions go in the source file `states_functions.cc`.

This program is divided into several files:
1. `states_functions.h` is a header file that declares the `State` class.  **Do not change this file.**
1. `states_functions.cc` is a source file that defines the member functions of the `State` class. Your work is to add working function bodies to this file (and a header comment).
