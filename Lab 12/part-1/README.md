...
1. With your partner, edit the `animated_gradient_functions.cc` source file using VS Code. Replace all the TODO comments with working code.
1. Compile your program with the `$ make` shell command. Use the **debug compile error** procedure to debug any compile errors.
1. Run your program with the `$ ./animated_gradient my_output_image.gif` shell command.
...
for (int current_image = 0; current_image < kNumberOfImages; current_image++) {
    ...
    blue = sin(blue_step * current_image);
    // column is the 'x' direction and row is the 'y' direction
