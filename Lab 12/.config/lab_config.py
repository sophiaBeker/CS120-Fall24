...
targets = 'animated_gradient make_message'.split()
...
lab = {
    ...
    # Configuration of target, source files, and header files for each part.
    # These are the files that will be checked for headers, format, and lint.
    # other_src and other_header are files that are needed for building and
    # are not graded/assessed.
...
# Typically, no changes need to be made below this line unless there is an
# offending clang-tidy rule which needs to be disabled.
...
global_makefile = {
    ...
    'darwin_GTESTLIBS': '-L /opt/local/libexec/llvm-18/lib/libc++ -L /opt/local/libexec/llvm-18/lib -L /opt/local/libexec/llvm-18/lib/libunwind -L /opt/local/lib -lgtest -lgtest_main -lunwind -rpath /opt/local/libexec/llvm-18/lib/libc++ -rpath /opt/local/libexec/llvm-18/lib -rpath /opt/local/libexec/llvm-18/lib/libunwind',
}
...
makefiles[0].update(
    {
        'linux_CXXFLAGS': '-D LINUX -I /usr/include/GraphicsMagick',
        'linux_LDFLAGS': '-lGraphicsMagick -lGraphicsMagick++',
        'darwin_CXXFLAGS': '-D OSX -I /opt/local/include/GraphicsMagick',
        'darwin_LDFLAGS': '-L /opt/local/lib -lGraphicsMagick -lGraphicsMagick++',
        'do_unit_tests': True,
        'do_format_check': True,
        'do_lint_check': True,
    }
